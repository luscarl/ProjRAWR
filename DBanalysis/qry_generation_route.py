...
def generateR(origin, orig_continent, destination):
    qry = f"""with rank as (
	select "Orig", "Dest", SUM("Total Pax") AS pax, AVG("Yield") as yield, AVG("Rev") as revenue,
	ROW_NUMBER() OVER (PARTITION BY "Orig", "Dest" ORDER BY SUM("Total Pax") DESC)
    ...
    eg_df.drop('row_number', axis = 1, inplace= True)
    return eg_df
